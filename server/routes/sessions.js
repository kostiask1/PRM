...
router.post("/", async (req, res, next) => {
	try {
		...
		const baseName = storage.sanitizeName(req.body?.name) || new Date().toISOString().slice(0, 10);
		...
		if (req.body?.data && typeof req.body.data === "object") session.data = req.body.data;
...
router.get("/:fileName", async (req, res, next) => {
	try {
		...
		if (!(await storage.exists(fullPath))) return res.status(404).json({ error: "Сесію не знайдено." });
...
router.patch("/:fileName", async (req, res, next) => {
	try {
		...
		if (!nextName) return res.status(400).json({ error: "Назва не може бути порожньою." });

		const nextFileName = await storage.ensureUniqueSessionFile(slug, nextName, fileName);
		const updated = {
			...current, ...req.body,
			name: nextName,
		...
		if (nextFileName !== fileName) {
			await storage.renameWithRetry(fullPath, storage.sessionPath(slug, nextFileName));
		}
...
router.post("/reorder", async (req, res, next) => {
	try {
		...
		for (const fileName of Object.keys(orders)) {
			const session = await storage.readJson(storage.sessionPath(slug, fileName));
