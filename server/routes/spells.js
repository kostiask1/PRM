...
router.get("/search", async (req, res, next) => {
	try {
		...
		for (const [sourceKey, fileName] of Object.entries(index)) {
			const data = await storage.readJson(path.join(storage.SPELLS_DIR, fileName));
			const spells = Array.isArray(data) ? data : (data.spell || data.spells || data.results || []);
			results.push(...spells.filter(s => 
...
router.get("/:source", async (req, res, next) => {
	try {
		...
		if (!fileName) return res.status(404).json({ error: "Джерело не знайдено." });
		const data = await storage.readJson(path.join(storage.SPELLS_DIR, fileName));
