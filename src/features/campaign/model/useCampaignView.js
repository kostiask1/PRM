...
export default function useCampaignView(props) {
	...
	const loadCharacters = useCallback(async () => {
		try {
			const data = await campaignApi.getEntities(campaign.slug, "characters");
	...
	const loadNpcs = useCallback(async () => {
		try {
			const data = await campaignApi.getEntities(campaign.slug, "npc");
	...
	const loadLocations = useCallback(async () => {
		try {
			const data = await campaignApi.getEntities(campaign.slug, "locations");
	...
	const saveToServer = useCallback(
		async (updates) => {
			...
			try {
				await campaignApi.updateCampaign(campaign.slug, updates);
			} catch (err) {
		...
		async (state) => {
			...
			await Promise.all([
				campaignApi.updateCampaign(
					campaign.slug,
					campaignHistoryPayload(state),
				),
				campaignApi.replaceEntities(
					campaign.slug,
					"characters",
					nextCharacters,
				),
				campaignApi.replaceEntities(campaign.slug, "npc", nextNpcs),
				campaignApi.replaceEntities(
					campaign.slug,
					"locations",
					nextLocations,
	...
	const persistEntitiesReorder = useCallback(
		async (type, nextEntities = []) => {
			...
			try {
				await campaignApi.replaceEntities(
					campaign.slug,
					type,
					orderedEntities,
				);
	...
	const handleCharacterNameBlur = async (id, updatedChar, oldName, newName) => {
		...
		try {
			const saved = sanitizeLoadedEntity(
				await campaignApi.updateEntity(campaign.slug, "characters", entity.slug, {
					...sanitizeEntityForSave(entity),
	...
	const handleDeleteCharacter = async (id) => {
		...
		if (!char) return;
		await campaignApi.deleteEntity(campaign.slug, "characters", char.slug);
	...
	const handleNpcNameBlur = async (id, updatedNpc, oldName, newName) => {
		...
		try {
			const saved = sanitizeLoadedEntity(
				await campaignApi.updateEntity(campaign.slug, "npc", entity.slug, {
					...sanitizeEntityForSave(entity),
					_updateMentionReferences: true,
					_mentionOldName: oldName,
				}),
	...
	const handleNpcDelete = async (id) => {
		...
		if (!npc) return;
		await campaignApi.deleteEntity(campaign.slug, "npc", npc.slug);
	...
	const handleLocationNameBlur = async (
		...
		try {
			const saved = sanitizeLoadedEntity(
				await campaignApi.updateEntity(campaign.slug, "locations", entity.slug, {
					...sanitizeEntityForSave(entity),
					_updateMentionReferences: true,
					_mentionOldName: oldName,
				}),
	...
	const handleLocationDelete = async (id) => {
		...
		if (!location) return;
		await campaignApi.deleteEntity(campaign.slug, "locations", location.slug);
		setLocations((prev) => prev.filter((item) => item.id !== id));
	};

	const loadSessions = useCallback(async () => {
		try {
			const data = await sessionApi.listSessions(campaign.slug);
			setSessions(data);
		} catch (err) {
		...
		try {
			const loadedEntries = await Promise.all(
				missingSessions.map(async (session) => [
					session.fileName,
					await sessionApi.getSession(campaign.slug, session.fileName),
				]),
			);
	...
	const handleCreateSession = async () => {
		...
		if (name === null) return;
		try {
			const newSession = await sessionApi.createSession(campaign.slug, name);
	...
	const handleDeleteCampaign = async () => {
		...
		try {
			const imageState = await campaignApi.campaignHasImages(campaign.slug);
		...
		try {
			await campaignApi.deleteCampaign(campaign.slug, {
				moveImagesToGeneral:
	...
	const handleRename = async () => {
		...
		if (name && name !== campaign.name) {
			try {
				const updated = await campaignApi.updateCampaign(campaign.slug, {
					name,
				});
				dispatch(requestCampaignsReloadAction());
	...
	const handleDeleteSession = async (session) => {
		if (
			...
			return;
		try {
			await sessionApi.deleteSession(campaign.slug, session.fileName);
			const data = await sessionApi.listSessions(campaign.slug);
			setSessions(data);
			setSessionDetails((prev) => {
				const next = { ...prev };
	...
	const handleExport = async () => {
		try {
			const blob = await campaignApi.exportCampaignArchive(campaign.slug);
			downloadBlob(
				blob,
	...
	const handleExportPartial = async (sections = []) => {
		try {
			const blob = await campaignApi.exportCampaignPartialArchive(
				campaign.slug,
				sections,
			);
			downloadBlob(
	...
	const handleImportPartial = async (file, sections = []) => {
		try {
			await campaignApi.importCampaignPartialArchive(
				campaign.slug,
				file,
				sections,
			);
			await Promise.all([loadCharacters(), loadNpcs(), loadLocations()]);
			const sessionsData = await sessionApi.listSessions(campaign.slug);
	...
	const handleSessionReorderDrop = useCallback(
		(nextSessions = sessions) => {
			...
			sessionApi.reorderSessions(campaign.slug, orders);
		},
			...
			try {
				await Promise.all(
					entityTypes.map(async (type) => {
						const entities = await campaignApi.getEntities(
							campaign.slug,
							type,
						);
