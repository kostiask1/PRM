...
import { MonsterFieldEditModal } from "../../../entities/bestiary/ui.js";
import {
	alert,
	confirm,
...
import BestiaryAiModals from "./BestiaryAiModals";
import BestiaryContent from "./BestiaryContent";
import MonsterAiActionModal from "./MonsterAiActionModal";
...
} from "../../../entities/ai/model.js";
import { loadAiModelOptions } from "../../ai/model.js";
import { filterBestiaryMonsters } from "../model.js";
import {
...
import { downloadJsonFile } from "../../../shared/lib/download.js";
...
export default function Bestiary({
	...
	renderAiAttachmentControls = null,
	renderAiResponseModal = null,
	...
	const restoreCustomMonsters = async (nextCustomMonsters, options = {}) => {
		const updated = await bestiaryApi.replaceCustomMonsters(nextCustomMonsters);
		...
		const loadInitialData = async () => {
			try {
				...
				const [sourcesData, legendaryData, favData] = await Promise.all([
					bestiaryApi.getSources(options),
					bestiaryApi.getLegendaryGroups(options),
					bestiaryApi.getFavorites(options),
				]);
	...
	useEffect(() => {
		...
		bestiaryApi
			...
			.then((nextFavorites) => {
				if (!controller.signal.aborted) setFavorites(nextFavorites);
			})
			.catch((error) => {
		...
		const loadData = async () => {
			...
			try {
				...
				const [officialData, customData] = await Promise.all([
					bestiaryApi.getData("all", options),
	...
	useEffect(() => {
		...
		const loadCustomData = async () => {
			try {
				const customData = await bestiaryApi.getCustomData({
	...
	const saveEditedCustomMonster = async (draftMonster) => {
		...
		try {
			if (fieldEditingMode === "create-based") {
				...
				const updated = await bestiaryApi.replaceCustomMonsters([
					...customMonsters,
					createdMonster,
				]);
	...
	const saveAiEditedCustomMonster = async () => {
		...
		try {
			const data = await aiApi.generateAi(
				{
					type: "custom-monster",
	...
	const restoreAiDraftResponse = async (
		...
		try {
			const result =
				mode === "undo"
					? await aiApi.undoAiResponse("bestiary", entry.id, {
							resourceIds: options.resourceIds,
						})
					: await aiApi.applyAiResponse("bestiary", entry.id, {
							resourceIds: options.resourceIds,
						});
	...
	const handleDeleteCustomMonster = async (monster) => {
		...
		try {
			const updatedCustomMonsters = await bestiaryApi.deleteCustomMonster(
				monster.name,
			);
			pushCustomUndoSnapshot(undoSnapshot);
			shouldAutoSelectMonsterRef.current = false;
	...
	const bestiaryModals = (
		<>
			<MonsterFieldEditModal
				...
				renderRulesReference={renderRulesReference}
			/>
