...
export default function useEncounterView() {
	...
	const loadEncounter = useCallback(
		async ({ retries = 3, resetHistory = true } = {}) => {
			try {
				const session = await sessionApi.getSession(campaign.slug, sessionId);
		...
		const loadEntityImages = async () => {
			try {
				const [characters, npcs] = await Promise.all([
					campaignApi.getEntities(campaign.slug, "characters"),
					campaignApi.getEntities(campaign.slug, "npc"),
				]);
		...
		(updatedEncounter, debounceMs = 0) => {
			...
			const performSave = async () => {
				try {
					...
					await sessionApi.updateSession(campaign.slug, sessionId, {
						...currentSession,
