...
import { SpellCard } from "../../../entities/spell/ui.js";
import { filterSpells } from "../model.js";
...
export default function Spells({
	...
	useEffect(() => {
		...
		const loadSources = async () => {
			try {
				const data = await spellApi.getSources({
	...
	useEffect(() => {
		...
		const loadData = async () => {
			setLoading(true);
			try {
				const combinedList = await spellApi.getData("all", {
					signal: controller.signal,
				});
				if (controller.signal.aborted) return;
