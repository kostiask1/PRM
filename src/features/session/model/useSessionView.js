...
import { idsEqual } from "../../../shared/lib/id.js";
...
export default function useSessionView() {
		...
		async ({ force = false } = {}) => {
			...
			try {
				const data = normalizeLoadedSession(
					await sessionApi.getSession(campaignSlug, sessionId),
				);
	...
	const moveSessionEntityToCampaign = async (type, id) => {
		...
		try {
			await campaignApi.createEntity(
				campaignSlug,
				type,
	...
	const handleOpenEncounter = async (scene, event = null) => {
		...
		if (!encounterId) {
			...
			await sessionApi.updateSession(campaignSlug, sessionId, {
				...session,
				data: nextData,
			});
	...
	const handleDeleteSessionAndBack = async () => {
		...
		if (
			...
			await sessionApi.deleteSession(campaignSlug, sessionId);
			handleBack();
			dispatch(requestCampaignsReloadAction());
		}
	};
