...
import DiceCalculator from "./ui/DiceCalculator";
import MainContent from "./router/MainContent";
import MessageBox from "./ui/MessageBox";
import Modal from "../components/common/Modal";
import CampaignEntityModalProvider from "./providers/CampaignEntityModalProvider";
...
import RulesReferenceModalHost from "./ui/RulesReferenceModalHost";
import { applyTheme, lang } from "../shared/config/index.js";
import {
	alert,
...
export default function App() {
		...
		const handleMentionPicker = async () => {
			...
			try {
				const [characters, npcs, locations] = await Promise.all([
					campaignApi.getEntities(activeCampaignSlug, "characters"),
					campaignApi
						.getEntities(activeCampaignSlug, "npc")
						.catch(() => []),
					campaignApi
						.getEntities(activeCampaignSlug, "locations")
						.catch(() => []),
				]);
	...
	const handleToggleCampaignStatus = async (campaign) => {
		...
		try {
			await campaignApi.updateCampaign(campaign.slug, {
			...
			children: (
				<CreateCampaignModalContent
					...
					onCreateCampaign={async (name) => {
						if (!name?.trim()) return;
						try {
							const newCampaign = await campaignApi.createCampaign(name.trim());
							dispatch(requestCampaignsReloadAction());
							handleClose();
					...
					onImportCampaign={async (file) => {
						try {
							await archiveApi.importArchive(file, "campaign");
							dispatch(requestCampaignsReloadAction());
							handleClose();
						} catch (err) {
							dispatch(
