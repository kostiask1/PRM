...
} from "../../../features/bestiary/index.js";
import { useEncounterView } from "../../../features/encounter/model.js";
...
} from "../../../entities/encounter/model.js";
import {
...
import { loadAiModelOptions } from "../../../features/ai/model.js";
...
const renderAiResponseModal = (props) => (
	<AiResponseModal
		{...props}
		renderRulesReference={renderRulesReferencePicker}
	/>
...
const renderAiAttachmentControls = (props) => (
	<AiAttachmentControls {...props} />
);
...
function EncounterPage() {
	...
	const { gridMonsters, gridRepresentativeByInstanceId } = useMemo(
		() => buildEncounterGridModel(view.encounter?.monsters || []),
	...
	const saveEditedMonsterFields = async (draftMonster) => {
		...
		try {
			let updatedMonster;
			if (mode === "edit") {
				updatedMonster = await bestiaryApi.updateCustomMonster(original.name, {
	...
	const restoreAiDraftResponse = async (
		...
		try {
			if (aiDraftMode === "local") {
				const result =
					mode === "undo"
						? await aiApi.undoAiResponse(campaign.slug, entry.id, {
								resourceIds: options.resourceIds,
							})
						: await aiApi.applyAiResponse(campaign.slug, entry.id, {
								resourceIds: options.resourceIds,
							});
	...
	const updateEncounterViewMode = (mode) => {
		...
		settingsApi.update({ encounterViewMode: nextMode }).catch((error) => {
	...
	const updateEncounterGridColumns = (columns) => {
		...
		settingsApi.update({ encounterGridColumns: nextColumns }).catch((error) => {
	...
	const handleCreatePlayer = async () => {
		...
		try {
			const created = await campaignApi.createEntity(
				campaign.slug,
				"characters",
				payload,
			);
	...
	return (
		<Panel className="EncounterView">
			...
			{view.showBestiary && (
				...
				>
					<Bestiary
						onAddMonster={view.handleAddMonster}
