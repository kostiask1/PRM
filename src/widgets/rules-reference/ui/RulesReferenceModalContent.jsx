...
import { rulesReferenceApi } from "../../../entities/rules-reference/api.js";
import { spellApi } from "../../../entities/spell/api.js";
import { getSpellMeta as formatSpellMeta } from "../../../entities/spell/meta.js";
import { Spells } from "../../../features/spells/index.js";
...
async function loadSpellReferenceItems(options = {}) {
	return spellApi.getData("all", options);
}

async function loadBestiaryReferenceItems(options = {}) {
	const [officialData, customData] = await Promise.all([
		bestiaryApi.getData("all", options),
		bestiaryApi
			.getCustomData(options)
			.catch((error) => (isAbortError(error) ? Promise.reject(error) : [])),
	]);
...
const REFERENCE_TABS = [
	{
		...
		load: (options) => rulesReferenceApi.getConditions(options),
		searchFields: ["name"],
	},
	{
		id: "diseases",
		...
		load: (options) => rulesReferenceApi.getDiseases(options),
	...
	},
	{
		...
		load: (options) => rulesReferenceApi.getSenses(options),
		searchFields: ["name"],
	},
	{
		id: "skills",
		...
		load: (options) => rulesReferenceApi.getSkills(options),
	...
	},
	{
		...
		load: (options) => rulesReferenceApi.getVariantRules(options),
