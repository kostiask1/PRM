...
import { setActiveSessionAction } from "../../../entities/session/model.js";
import { setActiveEncounterAction } from "../../../entities/encounter/model.js";
...
export default function AiAssistantPanel({
	...
	const refreshResponseHistoryStats = useCallback(async () => {
		...
		try {
			const stats = await aiApi.getAiResponsesStats(aiHistoryCampaign);
	...
	useEffect(() => {
		if (
			...
			sessionApi.listSessions(initialRoute.campaign).then(setSessionsList);
		}
	}, [
		isBestiary,
		isOpen,
	...
	useEffect(() => {
		...
		Promise.all([
			aiApi.listAiResponses(aiHistoryCampaign),
			aiApi.getAiResponsesStats(aiHistoryCampaign).catch((err) => {
				console.error("Failed to load AI response history stats", err);
				return null;
			}),
		])
		...
		(async () => {
			try {
				const sessions =
					sessionsList.length > 0
						? sessionsList
						: await sessionApi.listSessions(initialRoute.campaign);
				if (cancelled) return;
	...
	const deleteResponseHistoryEntry = async (entry) => {
		...
		try {
			const responses = await aiApi.deleteAiResponse(
	...
	const clearResponseHistory = async () => {
		...
		try {
			const responses = await aiApi.clearAiResponses(aiHistoryCampaign);
			setResponseHistory(Array.isArray(responses) ? responses : []);
			refreshResponseHistoryStats();
			closeGeneratedPrompt();
		} catch (err) {
	...
	const restoreAiHistoryEntry = async (entry, mode, options = {}) => {
		...
		try {
			...
			const result = isUndo
				? await aiApi.undoAiResponse(historyCampaign, entry.id, {
						resourceIds: options.resourceIds,
					})
				: await aiApi.applyAiResponse(historyCampaign, entry.id, {
						resourceIds: options.resourceIds,
					});
	...
	const handleSaveApiKey = async () => {
		...
		setError("");
		try {
			await aiApi.saveGeminiApiKey(apiKey);
			let result = null;
			for (let attempt = 0; attempt < 5; attempt++) {
				try {
					result = await aiApi.listAiModels();
					break;
				} catch (err) {
					if (attempt === 4) {
	...
	const toggleSessionDetails = async (sessionSlug) => {
		...
		if (!isExpanded && !contextConfig.sessions[sessionSlug]?.data) {
			setLoading(true);
			try {
				const fullData = await sessionApi.getSession(
					initialRoute.campaign,
					sessionSlug,
				);
				setContextConfig((prev) => ({
	...
	const retryResponseHistoryEntry = async (entry) => {
		...
		try {
			...
			const data = await aiApi.generateAi(retryPayload, {
				signal: controller.signal,
			});
			handleGeneratedAiData({
				data,
	...
	const loadBestiaryImagePromptData = async () => {
		if (imagePromptBestiaryDataLoadedRef.current) return;
		try {
			const data = await bestiaryApi.getCustomData();
			const monsters = Array.isArray(data?.monster)
				? data.monster
				: Array.isArray(data?.monsters)
					? data.monsters
