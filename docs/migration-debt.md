...
| MD-R02 / Phase 272 | Completed Bestiary widget UI decomposition | Moved Bestiary Browser-owned state and refs into private `widgets/bestiary-browser/ui/useBestiaryBrowserState.ts`. `BestiaryBrowser.tsx` retains UI orchestration. | Preserve state/ref initialization order, all setter/ref identities supplied to workflows and UI hosts, cleanup ownership, and no public widget entry expansion. | 545/545 tests; architecture maintained; `MD-R02` remains closed; `MD-R04` remains open |
